...
BUG FIXES aplicados neste arquivo:
  1. JSONResponse adicionado aos imports (estava faltando, causava NameError em todas as rotas de erro)
  2. edit_subscription_modal: removida importação de get_subscription (não existe) — query direto no DB
  3. edit_goal_modal: removida importação de get_goal (não existe) — query direto no DB
  4. admin_settings_post: SYSTEM_CONFIG não existe em config.py — simplificado para evitar AttributeError
  5. edit_card: card.last_four → card.last4 (nome correto do atributo no modelo)
  6. edit_subscription: sub_id → subscription_id (nome correto do parâmetro em subscriptions.py)
  7. Rotas GET de modal duplicadas removidas (estavam em htmx.py e web.py simultaneamente,
     causando comportamento imprevisível dependendo da ordem de registro em main.py)
"""
import uuid
import logging
...
async def login_post(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not user:
            return templates.TemplateResponse("login.html", {
                "request": request, "error": "E-mail ou senha incorretos", "email": email
            })

        if not verify_password(password, user.password_hash):
            return templates.TemplateResponse("login.html", {
                "request": request, "error": "Senha incorreta", "email": email
            })

        if not user.is_active:
            return templates.TemplateResponse("login.html", {
                "request": request, "error": "Conta inativa. Contate o administrador.", "email": email
            })
        ...
        if tenant and tenant.plan == "free" and tenant.trial_expires_at:
            ...
            if tenant.trial_expires_at < now:
                return templates.TemplateResponse("login.html", {
                    "request": request,
                    "error": "Seu período de teste expirou. Assine o plano PRO para continuar.",
                    "email": email,
                    "trial_expired": True,
        ...
        response.set_cookie("access_token", tokens.access_token, httponly=True, samesite="lax", path="/", max_age=86400 * 7)
        response.set_cookie("refresh_token", tokens.refresh_token, httponly=True, samesite="lax", path="/", max_age=86400 * 30)
        ...
        traceback.print_exc()
        return templates.TemplateResponse("login.html", {
            "request": request, "error": f"Erro ao fazer login: {str(e)}", "email": email
        })
...
async def forgot_password_post(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not email:
        return templates.TemplateResponse("forgot-password.html", {
            "request": request, "error": "E-mail é obrigatório"
        })
...
async def register_post(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    def render_error(error):
        return templates.TemplateResponse("register.html", {
            ...
            "name": name, "email": email, "workspace_name": workspace_name
        })

    if not name or not email or not password or not workspace_name:
        return render_error("Todos os campos são obrigatórios")
    if password != confirm_password:
        return render_error("As senhas não conferem")
    if len(password) < 6:
        return render_error("A senha deve ter pelo menos 6 caracteres")
    ...
    try:
        ...
        tenant = Tenant(
            name=workspace_name, slug=slug, plan="free", max_users=1,
            trial_start=now, trial_days=31, trial_expires_at=now + timedelta(days=31)
        )
        db.add(tenant)
        await db.flush()
        ...
        import traceback
        traceback.print_exc()
        return render_error(f"Erro ao criar conta: {str(e)}")
...
async def reset_password_post(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not token:
        return templates.TemplateResponse("reset-password.html", {"request": request, "error": "Token é obrigatório"})
    if new_password != confirm_password:
        return templates.TemplateResponse("reset-password.html", {"request": request, "error": "As senhas não conferem", "token": token})
    if len(new_password) < 6:
        return templates.TemplateResponse("reset-password.html", {"request": request, "error": "A senha deve ter pelo menos 6 caracteres", "token": token})
    ...
    return templates.TemplateResponse("reset-password.html", {
        "request": request, "success": "Senha alterada com sucesso! Agora você pode fazer login."
    })
...
async def setup_password_post(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if password != confirm:
        return templates.TemplateResponse("setup_password.html", {"request": request, "error": "As senhas não conferem", "token": token})
    if len(password) < 8:
        return templates.TemplateResponse("setup_password.html", {"request": request, "error": "A senha deve ter pelo menos 8 caracteres", "token": token})

    if token:
        result = await db.execute(select(User).where(User.reset_token == token))
        user = result.scalar_one_or_none()
        if not user or not user.reset_token_expires or user.reset_token_expires.replace(tzinfo=None) < datetime.now():
            return templates.TemplateResponse("setup_password.html", {"request": request, "error": "Token expirado ou inválido", "token": token})
    ...
    return templates.TemplateResponse("setup_password.html", {"request": request, "error": "Token necessário", "token": token})
...
async def dashboard_page(request: Request, month: int = None, year: int = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    for card in cards:
        spent_result = await db.execute(
            select(func.sum(Transaction.amount)).where(Transaction.card_id == card.id, Transaction.type == "expense")
        )
        ...
        cards_summary.append({
            "id": str(card.id), "name": card.name, "brand": card.brand, "last4": card.last4 or "****",
            "limit": float(card.limit_amount), "limit_fmt": fmt_money(card.limit_amount),
            "total_spent": float(total_spent), "total_spent_fmt": fmt_money(total_spent),
            "invoice": float(invoice), "invoice_fmt": fmt_money(invoice),
            "available": float(available), "available_fmt": fmt_money(available),
            "due_day": card.due_day, "close_day": card.close_day, "color": card.color,
    ...
    goals_summary = [{
        "id": str(g.id), "name": g.name, "target": float(g.target_amount), "current": float(g.current_amount),
        "target_fmt": fmt_money(g.target_amount), "current_fmt": fmt_money(g.current_amount),
        "progress": round((g.current_amount or 0) / (g.target_amount or 1) * 100, 1), "color": g.color or "#10B981"
    } for g in goals[:3]]

    from datetime import timedelta
    upcoming_date = now.date() + timedelta(days=7)
    subs_result = await db.execute(select(Subscription).where(
        Subscription.tenant_id == current_user.tenant_id, Subscription.is_active == True,
        Subscription.next_billing_date <= upcoming_date
    ))
    ...
    return templates.TemplateResponse("dashboard.html", {
        "request": request, "user": current_user, "now": datetime.now(),
        "month": month, "year": year, "month_label": get_month_label(month, year),
        **stats,
        ...
        "accounts_summary": accounts_summary, "total_balance": float(total_balance), "total_balance_fmt": fmt_money(total_balance),
        "cards_summary": cards_summary, "goals_summary": goals_summary, "upcoming_subs": upcoming_subs,
    })
...
async def transactions_page(request: Request, month: int = None, year: int = None, q: str = None, type: str = None, category_id: str = None, page: int = 1, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    return templates.TemplateResponse("transactions.html", {
        ...
        "transactions": [
            {
                "id": str(t.id), "description": t.description, "amount": t.amount,
                "amount_fmt": fmt_money(t.amount), "type": t.type, "date": t.date,
                "notes": t.notes, "source": t.source, "account_id": t.account_id,
                "card_id": t.card_id, "category_id": t.category_id,
        ...
        "subscriptions": [
            {
                ...
                # BUG FIX: next_date já enviado como string formatada para evitar .strftime em template
...
async def installments_page(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    for tx in all_tx:
        children = await db.execute(select(Transaction).where(Transaction.parent_id == tx.id))
    ...
    for tx in all_tx:
        ...
        installments.append({
            "id": str(tx.id), "description": tx.description, "name": tx.description,
            "total_amount": monthly * total, "total_installments": total,
            "paid_installments": paid, "installments": total, "paid": paid,
            "monthly_amount": monthly, "remaining": remaining, "remaining_fmt": fmt_money(remaining),
    ...
    return templates.TemplateResponse("installments.html", {
        ...
        "monthly_total": monthly_total, "monthly_total_fmt": fmt_money(monthly_total),
        "remaining_total": remaining_total, "remaining_total_fmt": fmt_money(remaining_total),
        "installments": installments, "cards": cards,
    })
...
async def create_installment(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    from app.models.transaction import Transaction as Tx, TransactionType, PaymentMethod
    ...
    try:
        ...
        root_tx = Tx(
            ...
            date=start_date, amount=per_amount, description=description,
            type=TransactionType.expense, payment_method=PaymentMethod.credit_card,
            category_id=uuid.UUID(category_id) if category_id else None,
            card_id=uuid.UUID(card_id), installments_total=installments,
            parent_id=None, installment_number=1,
        )
        db.add(root_tx)
        await db.flush()
        ...
        for i in range(1, installments):
            ...
            child_tx = Tx(
                tenant_id=current_user.tenant_id, user_id=current_user.id,
                date=datetime(year, month, day).date(), amount=per_amount,
                description=f"{description} ({i+1}/{installments})",
                type=TransactionType.expense, payment_method=PaymentMethod.credit_card,
                category_id=uuid.UUID(category_id) if category_id else None,
                card_id=uuid.UUID(card_id), installments_total=installments,
                parent_id=root_tx.id, installment_number=i + 1,
            )
            db.add(child_tx)
...
async def reports_page(request: Request, month: int = None, year: int = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    result = await db.execute(
        ...
        .group_by(Category.id, Category.name, Category.color).order_by(func.sum(Transaction.amount).desc())
    )
    rows = result.all()
    ...
    account_summary = []
    for acc in accounts:
        inc_acc = (await db.execute(select(func.coalesce(func.sum(Transaction.amount), 0)).where(Transaction.tenant_id == current_user.tenant_id, Transaction.account_id == acc.id, Transaction.type == TransactionType.income, Transaction.date >= start, Transaction.date < end))).scalar() or 0
        exp_acc = (await db.execute(select(func.coalesce(func.sum(Transaction.amount), 0)).where(Transaction.tenant_id == current_user.tenant_id, Transaction.account_id == acc.id, Transaction.type == TransactionType.expense, Transaction.date >= start, Transaction.date < end))).scalar() or 0
        account_summary.append({"name": acc.name, "income": float(inc_acc), "income_fmt": fmt_money(inc_acc), "expense": float(exp_acc), "expense_fmt": fmt_money(exp_acc), "balance": float(inc_acc - exp_acc), "balance_fmt": fmt_money(inc_acc - exp_acc), "current_balance": float(acc.balance), "current_balance_fmt": fmt_money(acc.balance)})

    cards = (await db.execute(select(Card).where(Card.tenant_id == current_user.tenant_id, Card.is_active == True))).scalars().all()
    card_summary = []
    for card in cards:
        spent_amt = float((await db.execute(select(func.coalesce(func.sum(Transaction.amount), 0)).where(Transaction.tenant_id == current_user.tenant_id, Transaction.card_id == card.id, Transaction.type == TransactionType.expense, Transaction.date >= start, Transaction.date < end))).scalar() or 0)
        limit_amt = float(card.limit_amount) if card.limit_amount else 0
        card_summary.append({"name": card.name, "spent": spent_amt, "spent_fmt": fmt_money(spent_amt), "limit": limit_amt, "limit_fmt": fmt_money(limit_amt), "available": limit_amt - spent_amt, "available_fmt": fmt_money(limit_amt - spent_amt)})
    ...
    top_transactions = [{"description": tx.description or "Sem descrição", "amount": float(tx.amount), "amount_fmt": fmt_money(tx.amount), "type": tx.type.value if tx.type else "expense", "date": tx.date.strftime("%d/%m") if tx.date else "-", "category": tx.name or "Sem categoria", "icon": tx.icon or "tag"} for tx in top_tx_result.all()]
    ...
    return templates.TemplateResponse("reports.html", {
        "request": request, "user": current_user, "month": month, "year": year,
        "month_label": get_month_label(month, year),
        "expense_breakdown": expense_breakdown, "income_breakdown": income_breakdown,
        "total_income": total_income, "total_income_fmt": fmt_money(total_income),
        "total_expense": total_expense, "total_expense_fmt": fmt_money(total_expense),
        "net_balance": net_balance, "net_balance_fmt": fmt_money(net_balance),
        "savings_rate": savings_rate, "account_summary": account_summary,
        "card_summary": card_summary, "comparison_data": comparison_data,
        "top_transactions": top_transactions,
        "monthly_labels": monthly_labels, "monthly_income": monthly_income, "monthly_expense": monthly_expense,
    })
...
async def goals_page(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    try:
        ...
        return templates.TemplateResponse("goals.html", {
            ...
            "goals": [
                {
                    "id": str(g.id), "name": g.name, "target": g.target_amount, "target_fmt": fmt_money(g.target_amount),
                    "current": g.current_amount, "current_fmt": fmt_money(g.current_amount),
...
async def categories_page(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    try:
        ...
        return templates.TemplateResponse("categories.html", {
            "request": request, "user": current_user,
            "categories": [{"id": str(c.id), "name": c.name, "icon": c.icon, "type": c.type, "color": c.color} for c in categories],
            "income_categories": [{"id": str(c.id), "name": c.name, "icon": c.icon, "type": c.type, "color": c.color} for c in income_cats],
            "expense_categories": [{"id": str(c.id), "name": c.name, "icon": c.icon, "type": c.type, "color": c.color} for c in expense_cats],
            "active_tab": "expense",
        })
    except Exception as e:
        import traceback
...
async def settings_page(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    try:
        ...
        return templates.TemplateResponse("settings.html", {
            "request": request, "user": current_user,
            "net_worth": float(net_worth), "net_worth_fmt": fmt_money(net_worth),
...
# ── Account & Card POST handlers ──────────────────────────────────────────────

@router.post("/settings/accounts/new")
async def create_account_web(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    try:
        acc_type_str = form.get("type", "checking")
        try:
            account_type = AccountType(acc_type_str)
        except ValueError:
        ...
        body = AccountCreate(
            ...
            currency=form.get("currency", "BRL"), color=form.get("color", "#10B981"),
        ...
        return templates.TemplateResponse("partials/_account_modal.html", {
            "request": request, "user": current_user,
            "account": account_to_dict(account), "success": True,
            "institutions": (await db.execute(select(Institution).where(Institution.is_active == True).order_by(Institution.name))).scalars().all(),
        })
    except Exception as e:
        import traceback
...
async def edit_account_web(account_id: str, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    from app.routers.accounts_cards import account_to_dict, AccountType
    ...
    try:
        ...
        if name:
            account.name = name
        acc_type_str = form.get("type")
        if acc_type_str:
            try:
                account.type = AccountType(acc_type_str)
            except ValueError:
                pass
        if form.get("balance"):
...
async def delete_account_web(account_id: uuid.UUID, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
...
async def delete_card_web(card_id: uuid.UUID, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
...
async def create_card_web(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    if not allowed:
        ...
        return templates.TemplateResponse("partials/_card_modal.html", {
            ...
            "institutions": inst_result.scalars().all(), "error": error,
        })
    try:
        inst_id = form.get("institution_id")
        body = CardCreate(
        ...
        return templates.TemplateResponse("partials/_card_modal.html", {
            "request": request, "user": current_user, "card": card_to_dict(card), "success": True,
            "institutions": (await db.execute(select(Institution).where(Institution.is_active == True).order_by(Institution.name))).scalars().all(),
        })
    except Exception as e:
        import traceback
        ...
        return templates.TemplateResponse("partials/_card_modal.html", {
            "request": request, "user": current_user, "card": None,
            "institutions": inst_result.scalars().all(), "error": str(e),
        })


@router.post("/settings/cards/{card_id}/edit")
async def edit_card_web(card_id: str, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
...
# ── Transaction POST handlers ─────────────────────────────────────────────────

@router.post("/transactions/new")
async def create_transaction_web(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    source_to_payment = {"pix": PM.pix, "bank": PM.transfer, "credit_card": PM.credit_card, "debit_card": PM.debit_card, "cash": PM.money, "other": PM.money}
    pay_method = source_to_payment.get(form.get("source", "cash"), PM.money)
    ...
    auto_tithe = prefs.auto_tithe if prefs else False
    ...
    try:
        ...
        return templates.TemplateResponse("partials/_tx_modal.html", {"request": request, "user": current_user, "tx": tx, "success": True, "categories": [], "accounts": [], "cards": [], "today": datetime.now().date().isoformat(), "tx_type": "expense"})
    except Exception as e:
        import traceback
        traceback.print_exc()
        return JSONResponse(content={"error": str(e)}, status_code=400)
...
async def edit_transaction_web(tx_id: str, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    try:
        ...
        tx = await update_transaction(transaction_id=uuid.UUID(tx_id), body=body, db=db, current_user=current_user)
        return templates.TemplateResponse("partials/_tx_modal.html", {"request": request, "user": current_user, "tx": tx, "success": True, "categories": [], "accounts": [], "cards": [], "today": datetime.now().date().isoformat(), "tx_type": "expense"})
    except Exception as e:
        import traceback
        traceback.print_exc()
...
async def create_category_web(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    try:
        body = CategoryCreate(name=form.get("name", ""), icon=form.get("icon", "folder"), color=form.get("color", "#6B7280"), type=cat_type)
...
async def edit_category_web(cat_id: str, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
...
# ── Subscription POST handlers ────────────────────────────────────────────────

@router.post("/subscriptions/new")
async def create_subscription_web(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    billing_date = datetime.strptime(billing_date_str, "%Y-%m-%d").date() if billing_date_str else datetime.now().date()
    sub_type_str = form.get("type", "expense")
    sub_type_enum = SubscriptionType.income if sub_type_str == "income" else SubscriptionType.expense

    try:
        body = SubscriptionCreate(
            name=form.get("name", ""), amount=Decimal(str(form.get("amount", 0) or 0)),
            billing_day=int(form.get("billing_day", 1) or 1),
            category_id=uuid.UUID(form.get("category_id")) if form.get("category_id") else None,
            account_id=uuid.UUID(form.get("account_id")) if form.get("account_id") else None,
            card_id=uuid.UUID(form.get("card_id")) if form.get("card_id") else None,
            next_billing_date=billing_date,
            # BUG FIX: type deve ser string, não enum — SubscriptionCreate.type: str
            type=sub_type_str,
            is_active=form.get("active", "1") in ("1", "on", "true"),
        )
        sub = await api_create_subscription(body=body, db=db, current_user=current_user)
        return templates.TemplateResponse("partials/_sub_modal.html", {"request": request, "user": current_user, "sub": sub, "success": True, "categories": [], "accounts": [], "cards": [], "today": datetime.now().strftime("%Y-%m-%d")})
    except Exception as e:
        import traceback
        traceback.print_exc()
        return JSONResponse(content={"error": str(e)}, status_code=400)
...
async def edit_subscription_web(sub_id: str, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    # BUG FIX: anteriormente importava get_subscription que não existe. Agora usa update_subscription diretamente.
    # BUG FIX: parâmetro renomeado de sub_id para subscription_id para corresponder à assinatura de update_subscription
    ...
    try:
        ...
        # BUG FIX: subscription_id é o nome correto do parâmetro
        sub = await update_subscription(subscription_id=uuid.UUID(sub_id), body=body, db=db, current_user=current_user)
        return templates.TemplateResponse("partials/_sub_modal.html", {"request": request, "user": current_user, "sub": sub, "success": True, "categories": [], "accounts": [], "cards": [], "today": datetime.now().strftime("%Y-%m-%d")})
    except Exception as e:
        import traceback
        traceback.print_exc()
...
async def create_goal_web(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    try:
        ...
        body = GoalCreate(
            ...
            account_id=uuid.UUID(form.get("account_id")) if form.get("account_id") else None,
            deadline=deadline, color=form.get("color", "#10B981"),
            notify_on_complete=form.get("notify_on_complete") in ("1", "on", "true"),
            notify_on_deadline=form.get("notify_on_deadline") in ("1", "on", "true"),
        )
        goal = await api_create_goal(goal_in=body, db=db, current_user=current_user)
        return templates.TemplateResponse("partials/_goal_modal.html", {
            "request": request, "user": current_user,
            "goal": {"id": str(goal.id), "name": goal.name, "target_amount": float(goal.target_amount), "current_amount": float(goal.current_amount), "deadline": goal.deadline, "color": goal.color},
...
async def edit_goal_web(goal_id: str, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    try:
        ...
        goal = await update_goal(goal_id=uuid.UUID(goal_id), goal_in=body, db=db, current_user=current_user)
        return templates.TemplateResponse("partials/_goal_modal.html", {
            "request": request, "user": current_user,
...
async def deposit_goal_web(goal_id: str, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    from app.models.transaction import Transaction as Tx, TransactionType, TransactionSource, PaymentMethod
    ...
    try:
        ...
        if source_account_id:
            ...
            if source_account and source_account.balance >= amount:
                tx = Tx(tenant_id=current_user.tenant_id, user_id=current_user.id, date=datetime.now().date(), amount=amount, description=f"Depósito na meta: {goal.name}", type=TransactionType.expense, payment_method=PaymentMethod.transfer, source=TransactionSource.manual, account_id=source_account.id, notes=note or f"Depósito para meta: {goal.name}")
                db.add(tx)
...
async def withdraw_goal_web(goal_id: str, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    from app.models.transaction import Transaction as Tx, TransactionType, TransactionSource, PaymentMethod
    ...
    try:
        ...
        if target_account_id:
            ...
            if target_account:
                tx = Tx(tenant_id=current_user.tenant_id, user_id=current_user.id, date=datetime.now().date(), amount=amount, description=f"Saque da meta: {goal.name}", type=TransactionType.income, payment_method=PaymentMethod.transfer, source=TransactionSource.manual, account_id=target_account.id, notes=note or f"Saque da meta: {goal.name}")
                db.add(tx)
...
async def options_page(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    from app.models.user import UserPreferences
    prefs_result = await db.execute(select(UserPreferences).where(UserPreferences.user_id == current_user.id))
    prefs = prefs_result.scalar_one_or_none()
    tenant_result = await db.execute(select(Tenant).where(Tenant.id == current_user.tenant_id))
    tenant = tenant_result.scalar_one_or_none()
    return templates.TemplateResponse("options.html", {"request": request, "user": current_user, "prefs": prefs, "tenant": tenant})
...
async def options_profile(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    current_user.email = form.get("email", "").lower()
    await db.commit()
...
async def options_password(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    if not verify_password(current_password, current_user.password_hash):
        return templates.TemplateResponse("options.html", {"request": request, "user": current_user, "error_password": "Senha atual incorreta"})
    if new_password != confirm_password:
        return templates.TemplateResponse("options.html", {"request": request, "user": current_user, "error_password": "As senhas não conferem"})
    if len(new_password) < 8:
        return templates.TemplateResponse("options.html", {"request": request, "user": current_user, "error_password": "A nova senha deve ter pelo menos 8 caracteres"})
...
async def options_notifications(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    form = await request.form()
    prefs_result = await db.execute(select(UserPreferences).where(UserPreferences.user_id == current_user.id))
    prefs_obj = prefs_result.scalar_one_or_none()

    values = {
        "email_notifications": form.get("email_notif") == "1",
        "monthly_reports": form.get("monthly_report") == "1",
    ...
    if prefs_obj:
        for k, v in values.items():
            setattr(prefs_obj, k, v)
    else:
        prefs_obj = UserPreferences(user_id=current_user.id, **values)
        db.add(prefs_obj)
...
async def options_preferences(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    if prefs_obj:
        prefs_obj.currency = form.get("currency", "BRL")
    else:
        prefs_obj = UserPreferences(user_id=current_user.id, currency=form.get("currency", "BRL"))
        db.add(prefs_obj)
...
async def options_workspace(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    if tenant_obj:
        tenant_obj.name = form.get("workspace_name", "")
        await db.commit()
...
async def export_page(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    return templates.TemplateResponse("export.html", {
        "request": request, "user": current_user,
        "can_export": allowed, "plan_error": error,
        "plan_name": "Free" if limits == PLAN_LIMITS["free"] else "Pro",
        "date_from": date(today.year, 1, 1).isoformat(), "date_to": today.isoformat(), "data_type": "all",
    })
...
async def import_page(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    return templates.TemplateResponse("import.html", {
        "request": request, "user": current_user,
        "can_import": allowed, "plan_error": error,
        "plan_name": "Free" if limits == PLAN_LIMITS["free"] else "Pro",
        "date_from": date(today.year, 1, 1).isoformat(), "date_to": today.isoformat(),
    })
...
async def get_notifications(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_user)):
    ...
    try:
        existing = await db.execute(select(Notification).where(Notification.user_id == current_user.id).limit(1))
        ...
        return templates.TemplateResponse("partials/_notifications.html", {
            "request": request, "user": current_user, "unread_count": unread_count,
            "notifications": [{"id": str(n.id), "title": n.title, "message": n.message, "type": n.type, "is_read": n.is_read, "link": n.link, "created_at": n.created_at.strftime("%d/%m/%Y %H:%M")} for n in notifications],
        })
    except Exception as e:
        import traceback
        traceback.print_exc()
...
async def admin_page(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_superadmin)):
    ...
    plans = [{"id": pid, **features} for pid, features in PLAN_FEATURES.items()]

    return templates.TemplateResponse("admin.html", {
        "request": request, "user": current_user,
        # BUG FIX: 'plans' agora é passado — o template admin.html faz {% for plan in plans %}
        "plans": plans,
        "tenants": [
            {
                "id": str(t.id), "name": t.name, "plan": t.plan, "active": t.is_active,
        ...
        "all_users": [
            {
                ...
                "tenant_name": (await db.execute(select(Tenant.name).where(Tenant.id == u.tenant_id))).scalar_one_or_none() if u.tenant_id else None,
            }
            for u in all_users
        ],
        "system_stats": {"total_tenants": total_tenants, "active_users": active_users, "transactions_30d": total_tx, "mrr_fmt": "R$ 0"},
    })
...
async def admin_settings_post(request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_superadmin)):
    """
    BUG FIX: settings.SYSTEM_CONFIG não existe em config.py, causava AttributeError.
    Configurações de sistema não são persistidas (seriam necessárias uma tabela no DB
    ou variável de ambiente). Por ora, apenas confirma o recebimento sem persistir.
    Para persistência real, implemente uma tabela 'system_config' no banco de dados.
    """
    form = await request.form()
    logger.info(f"Admin settings update requested: maintenance={form.get('maintenance_mode')}, open_reg={form.get('open_registration')}")
    return RedirectResponse(url="/admin?success=settings&note=config_not_persisted", status_code=302)
